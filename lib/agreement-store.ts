...
import { defaultValues, type AgreementValues } from "./agreement-fields";
...
export function emptyAgreement(dealId: string): AgreementRecord {
  return {
    dealId,
    // Seeded with the house standards - 12% 위약벌, 5 years, 3 copies - so a
    // new agreement starts from SparkLabs' position rather than blank.
...
export async function readAgreement(dealId: string): Promise<AgreementRecord> {
  ...
  try {
    ...
    const { values, updatedBy, updatedAt } = raw as Partial<AgreementRecord>;
    ...
    return {
      ...
      values: { ...defaultValues(), ...cleanValues(values) },
